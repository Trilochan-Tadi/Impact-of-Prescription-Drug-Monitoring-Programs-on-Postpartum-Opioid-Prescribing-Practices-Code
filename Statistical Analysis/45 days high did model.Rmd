...
```{r}
cohort_high <- cohort_data_nodup %>%
  semi_join(df_high_ids, by = "pat_id_p")
```
...
medical_comb_high <- medical_comb %>%
  semi_join(df_high_ids, by = "pat_id_p") %>%
...
data_cs_filtered_h <- data_cs_h %>%
  filter(T >= 2006 & T<=2015)
cohorts_att <- 2007:2015

d_full_h <- data_cs_filtered_h %>%
  filter(
    T >= 2006 & T <= 2015,
...
```{r}
# step 9e C&S model outome days supply
...
```{r}
data_cs_vag_high_filtered <-data_cs_filtered_h %>%
  filter(delivery == "Vaginal")

data_cs_vag_high_filtered <- data_cs_vag_high_filtered %>%
  filter(T >= 2006 & T<=2015)
cohorts_att <- 2007:2015

d_full_vag_h <- data_cs_vag_high_filtered %>%
  filter(
    T >= 2006 & T <= 2015,
    G == 0L | G %in% cohorts_att | G > 2014
  )
...
```{r}
data_cs_ces_high_filtered <-data_cs_filtered_h %>%
  filter(delivery == "Cesarean")
data_cs_ces_high_filtered <- data_cs_ces_high_filtered %>%
  filter(T >= 2006 & T<=2015)
cohorts_att <- 2007:2015

d_full_ces_h <- data_cs_ces_high_filtered %>%
  filter(
    T >= 2006 & T <= 2015,
    G == 0L | G %in% cohorts_att | G > 2014
  )
